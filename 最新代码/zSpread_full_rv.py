...
src_files = ['每日-进出口债收盘历史.csv','每日-国开债收盘历史.csv','每日-农发债收盘历史.csv']
...
def check_spread(spread_trades: pd.Series, spread_sigal: pd.Series, code: str, pnlY: float, left_year, side: str, trades_time: pd.Series):
    ...
    for indx, val in spread_sigal.items():
        if val > min_val:
            ...
            print("%%%%%%%%%%%% {5} for-code {0} on {7} zSpread {8}, total-PnL {1}, diff-zSpread {2}, bid-ask-spread {3}, zs-b-a_sprd {6} left {4} "
                  .format(code, pnlY, val - min_val, b_a_spread, left_year, side, val - min_val - b_a_spread, trades_time.max(), spread_sigal.max()))
            return True
    return False
    pass
...
def get_rv_signal(src_file: str):
    ...
    filename= str(uuid.uuid4().hex) + "_zSpread_full_"+ src_file + ".xlsx"
